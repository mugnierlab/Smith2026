...
raw_expt1 <- readxl::read_xlsx("JAC21-4mouse_collectionExpt1.xlsx",
                               sheet = "Parasitemia")
...
dates_expt1 <- expt1 %>%
  ...
  dplyr::mutate(mouse = c("E1M6", "E1M7", "E1M8", "E1M9", "E1M10", "E1M1", "E1M2", "E1M3", "E1M4", "E1M5"),
                ...
                experiment = "E1") %>%
  dplyr::mutate(D4 = na_if(D4, "NA")) %>%
  dplyr::mutate(D6 = na_if(D6, "NA")) %>%
  dplyr::mutate(D8 = na_if(D8, "NA")) %>%
  dplyr::mutate(D10 = na_if(D10, "NA")) %>%
  dplyr::mutate(D12 = na_if(D12, "NA")) %>%
  dplyr::mutate(D14 = na_if(D14, "NA")) %>%
  dplyr::mutate(D15 = na_if(D15, "NA")) %>%
...
raw_expt2 <- readxl::read_xlsx("JAC21-4mouse_collectionExpt2.xlsx",
                               sheet = "Parasitemia")
...
raw_expt3 <- readxl::read_xlsx("JAC21-4mouse_collectionExpt3.xlsx",
                               sheet = "Parasitemia")
