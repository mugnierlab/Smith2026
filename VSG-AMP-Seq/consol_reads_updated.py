...
def file_breakout(file_name):
    ...
    print(file_name)
    os.system("cd-hit-est -i " + file_name  + "  -o ./umi_fastas/consol_umis.fa -c 0.92 -n 8 -M 16000 -d 0 -T 16")
    consensus_umis = {}
    with open("./umi_fastas/consol_umis.fa", "r") as umis:
        for line in umis:
            if ">" in line:
                consensus_umis[line.strip().split(":")[-1].split("\\1")[0]] = 0
    with open("./umi_fastas/consol_umis.fa.clstr", "r") as clusters:
        umi_codes = {}
        umi_group = []
