...
cluster_dict = {}
for file_name in glob.glob("./cluster_uclust/Lister*"):
    with open(file_name, "r") as input_file:
        for line in input_file:
            if line.startswith("VSG"):
                continue
            try:
                cluster_dict[line.split(",")[1].split("cluster")[1]].append(line.strip().split(",")[2])
            except KeyError:
                cluster_dict[line.split(",")[1].split("cluster")[1]] = [line.strip().split(",")[2]]
